...
{
	subj = new cbop::Polygon;
	clip = new cbop::Polygon;
...
void MainWindow::openSubject ()
{
	...
	QString fileName = QFileDialog::getOpenFileName (this, title, "/home/fmartin/src/inv/clipping/polygons");
	if (!fileName.isEmpty ())
...
void MainWindow::computeBooleanOperation ()
{
	...
	if (dialog.exec ()) {
		result->clear ();
		cbop::compute (*subj, *clip, *result, dialog.operation ());
