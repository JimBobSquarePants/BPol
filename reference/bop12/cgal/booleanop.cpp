...
bool SweepEventComp::operator() (const SweepEvent* e1, const SweepEvent* e2)
{
	...
	return e1->above (e2->otherEvent->point);
}
...
bool SegmentComp::operator() (SweepEvent* le1, SweepEvent* le2)
{
	if (le1 == le2)
		return false;
	if (!le1->line.has_on (le2->point) || !le1->line.has_on (le2->otherEvent->point)) {
...
{
	CGAL::Bbox_2 subjectBB = subject.bbox ();     // for optimizations 1 and 2
	CGAL::Bbox_2 clippingBB = clipping.bbox ();   // for optimizations 1 and 2
...
bool BooleanOpImp::trivialOperation (const CGAL::Bbox_2& subjectBB, const CGAL::Bbox_2& clippingBB)
{
	...
	if (! CGAL::do_overlap (subjectBB, clippingBB)) {
...
/*	if (s.degenerate ()) // if the two edge endpoints are equal the segment is dicarded
	...
	if (s.min () == s.source ()) {
		e2->left = false;
		e1->line = e2->line = s.supporting_line ();
	} else {
		e1->left = false;
		e1->line = e2->line = s.opposite (). supporting_line ();
	}
	eq.push (e1);
	eq.push (e2);
}
...
//		return 0;
	...
	CGAL::Object obj = CGAL::intersection (le1->segment (), le2->segment ());
	const Point_2* point = CGAL::object_cast<Point_2> (&obj);
	const Segment_2* segment = CGAL::object_cast<Segment_2> (&obj);

	if (!point && !segment)
		return 0; // no intersection

	if ((point) && ((le1->point == le2->point) || (le1->otherEvent->point == le2->otherEvent->point)))
		return 0; // the line segments intersect at an endpoint of both line segments

	if (segment && le1->pol == le2->pol) {
	...
	if (point) {
		if (le1->point != *point && le1->otherEvent->point != *point)  // if the intersection point is not an endpoint of e1->segment ()
			divideSegment (le1, *point);
		if (le2->point != *point && le2->otherEvent->point != *point)  // if the intersection point is not an endpoint of e2->segment ()
			divideSegment (le2, *point);
		return 1;
	}
	// The edges overlap
