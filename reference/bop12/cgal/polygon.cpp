...
bool Contour::counterclockwise ()
{
	if (_precomputedCC)
		return _CC;
	_precomputedCC = true;
	return _CC = CGAL::orientation_2 (begin (), end ()) == CGAL::COUNTERCLOCKWISE;
}
...
std::ostream& bop::operator<< (std::ostream& o, Contour& c)
{
	o << c.nvertices () << '\n';
...
CGAL::Bbox_2 Polygon::bbox () const
{
	if (ncontours () == 0)
		return CGAL::Bbox_2 ();
	CGAL::Bbox_2 bb = contours[0].bbox ();
...
std::ostream& bop::operator<< (std::ostream& o, Polygon& p)
{
...
std::istream& bop::operator>> (std::istream& is, Polygon& p)
{
	// read the contours
	int ncontours;
	double px, py;
...
void Polygon::computeHoles ()
{
	...
	for (unsigned i = 0; i < ncontours (); i++) {
		...
		for (unsigned j = 0; j < contour (i).nedges (); j++) {
			...
			if (se1->point.x () < se2->point.x ()) {
				se2->left = false;
				se1->inOut = false;
				se1->line = se2->line = s.supporting_line ();
			} else {
				se1->left = false;
				se2->inOut = true;
				se1->line = se2->line = s.opposite (). supporting_line ();
			}
			evp.push_back (se1);
			evp.push_back (se2);
		}
	...
	for (unsigned int i = 0; i < evp.size () && nprocessed < ncontours (); i++)  {
		...
		if (e->left) { // the segment must be inserted into SL
		...
		} else { // the segment must be removed from SL
