...
DrawStepByStep::DrawStepByStep (const Polygon& subj, const Polygon& clip, bop::BooleanOpImp* boip, QWidget* parent) : 
...
void DrawStepByStep::drawSegment (const bop::SweepEvent* se) const
{
	drawPoint (inexactPoint (se->point));
	drawPoint (inexactPoint (se->otherEvent->point));
}
...
//	std::cout << "Start draw..." << std::endl;
	...
	glBegin (GL_LINES);
		// show sl events
		glColor3f (0, 1, 1);
		for (bop::BooleanOpImp::const_sl_iterator it = boi->beginSL (); it != boi->endSL (); ++it)
			drawSegment (*it);
		// show sortedEvents
		glColor3f (1, 0.6, 0.6);
		for (bop::BooleanOpImp::const_sortedEvents_iterator it = boi->beginSortedEvents (); it != boi->endSortedEvents (); ++it)
			if (((*it)->left && (*it)->inResult) || (!(*it)->left && (*it)->otherEvent->inResult))
				drawSegment (*it);
		// show out
		glColor3f (0, 0, 1);
		for (bop::BooleanOpImp::const_out_iterator it = boi->beginOut (); it != boi->endOut (); ++it) {
			drawSegment (*it);
/*			if ((*it)->prevInside) {
				glColor3f (0, 1, 0);
...
/*		glColor3f (0, 1, 0);
	...
	glColor3f (1, 0, 0);
	glBegin (GL_POINTS);
		drawPoint (inexactPoint (boi->currentPoint ()));
	glEnd ();
